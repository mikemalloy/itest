...
So resolve at run time: run `terraform show -json` in the configured Terraform
directory and look up the address. `conftest.py` does this once per session and
exposes it as fixtures.

Write this file to `itest_tests/conftest.py` on first generation. It reads
`tf_dir`, profile, and region from `.itest/skill-answers.yaml` — **never
hardcode an account, profile, region, or directory into it.**
...
itest-implementer skill. Nothing here is bound to an account or region.
"""
