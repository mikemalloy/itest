...
Each fixture is sanitized `terraform show -json` state from a reference stack
(aws-samples/serverless-patterns, or a terraform-aws-modules example) that was
applied, implemented via the bundled skill, verified green against a live
account, and destroyed. If a detector change alters these counts, the table
must change with it.
"""
...
EXPECTED = {
    ...
    # terraform-aws-modules/ecs "fargate" example: ALB with a blue/green
    # listener-rule pair -> two target groups -> one ECS service.
    "ecs-fargate-alb.json": {"iam_edge": 6, "lb_edge": 6},
}
